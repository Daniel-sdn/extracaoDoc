...
# python -m modules.trata_model

def get_template_version(qualquer_df, model):
    row_frame = utils.filtrar_df(qualquer_df, model=model)
...
def get_coordinates_filter(pdf_pesquisavel_map, model, tipo, label, section):
    ...
    row_frame = utils.filtrar_df(qualquer_df, model=model, type=tipo, label=label, section_json=section)
